...
mod tests {
    ...
    async fn retention_deletes_chunk_by_chunk_with_verdicts_explicit() {
        ...
        assert_eq!(deleted as i64, DELETE_CHUNK + 1, "a second chunk finished the job");
        assert_eq!(counts(&pool).await, (2, 2), "verdicts went with their rows, no cascade needed");

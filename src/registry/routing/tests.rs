...
use super::*;
use crate::domain::{RouteEffect, RepoSpec, Visibility};
...
fn stored(name: &str, patterns: &[&str], effect: RouteEffect) -> StoredRule {
    StoredRule {
        name: name.to_string(),
        format: Format::Npm,
...
async fn a_pinned_scope_never_leaves_for_an_upstream() {
    ...
    install(
        &mut fx,
        &[stored("acme", &["@acme/*"], RouteEffect::Members(vec![internal]))],
        1,
    );
...
async fn a_capital_letter_does_not_walk_around_a_rule() {
    let mut fx = fixture().await;
    install(&mut fx, &[stored("acme", &["@acme/*"], RouteEffect::AnyHosted)], 1);
...
async fn rules_intersect_and_an_exception_reopens_one_spelling() {
    let mut fx = fixture().await;
    let mut rule = stored("acme", &["@acme/*"], RouteEffect::AnyHosted);
    rule.except = vec!["@acme/public-ui".to_string()];
    install(
        &mut fx,
        &[rule, stored("secrets", &["@acme/secret*"], RouteEffect::Deny)],
        1,
    );
...
async fn a_snapshot_nobody_refreshed_closes_the_protected_surface() {
    let mut fx = fixture().await;
    let rules = [stored("acme", &["@acme/*"], RouteEffect::AnyHosted)];
...
async fn an_unreadable_rule_set_refuses_to_load() {
    let broken = stored("bad", &["a\\*b"], RouteEffect::Deny);

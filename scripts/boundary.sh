...
declare_row storage-delete max    8 'storage\.delete(_batch)?\(' plain src '*.rs' src/app/reclaim.rs src/testing # A1 C5bis: ReclaimOrphans deletes shared keys, private owners their own; left: OCI segments (private, 1), OCI manifest orphans and blob delete (S4, 3), promote compensation, proxy purge and miss, cache sweep (S3, 4)

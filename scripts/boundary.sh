...
declare_row stray-sql      max  64 'sqlx::query'             plain src '*.rs' src/db src/adapters # 78 -> 64: the cleanup sweeps run through PackageStore; the four left in telemetry/cleanup.rs seed the policy report, which has no port until 8b. covers _as and _scalar
...
declare_row dialect-rs     max   7 'datetime\(|julianday\(|strftime\(|AUTOINCREMENT|INSERT OR ' plain src '*.rs' src/db src/adapters/sqlite # 13 -> 7: the retention predicates bind the caller's clock too, so telemetry/cleanup.rs spells no date arithmetic at all
...
declare_row unit-tests     min 267 '#\[(tokio::)?test\]'     plain src '*.rs'                     # 266 -> 267: the retention bound's keep side. 255 -> 266: the walk state machine, the resolver's statuses, the depth cap, the failing grant lookup and the three hosted OCI reads. Floors: the suite may be rebalanced, not shrunk (7.5 rule 3)
declare_row integ-tests    min 302 '#\[(tokio::)?test\]'     plain tests '*.rs' # 300 -> 302: the delete-version cascade and the retention predicate, against both adapters. 278 -> 300: proxy_cache_contract! and package_contract! against two adapters, the user and token timestamp surfaces, and npm's browse and empty-query arms

...
declare_row storage-delete max    2 'storage\.delete(_batch)?\(' plain src '*.rs' src/app/reclaim.rs src/app/place.rs src/app/place_tests.rs src/testing # A1 C5bis: ReclaimOrphans deletes shared keys, place_shared its drafts, private owners their own (the placement tests stand in for a reclaimer); the two left are OCI segments, private to their session: a lost chunk's own, and the completion winner's

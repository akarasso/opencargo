...
declare_row integ-tests    min 363 '#\[(tokio::)?test\]'     plain tests '*.rs' # 362 -> 363: the PyPI policy row (1). 355 -> 362: PyPI proxy and group (7). 349 -> 355: hosted PyPI over HTTP (6). 341 -> 349: pypi_contract! (8). 336 -> 339: M1 in reclaim_contract! (3). 322 -> 336: reclaim_contract! (14, on the fake and on SQLite). 321 -> 322: stale Bearer with real pnpm. 317 -> 321: cascade_contract! gains the manifest cascade -- the orphan set, the unknown manifest, the re-push that replaces its layers and the upload ledger -- asserted against both halves
